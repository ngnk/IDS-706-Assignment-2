...
## 11) Customization & next steps

- **Swap commodities:** Filter to a single commodity with `df[df["Commodity"] == "Gold"]` and re‑run.
- **Change features:** Try longer/shorter moving averages, volatility windows, or macro features.
- **Time‑series splits:** Replace random/chronological splits with **rolling** or **walk‑forward** validation.
- **Model zoo:** Try `RandomForestClassifier/Regressor`, `XGBoost`, or linear models with regularization.
- **Save models:** Use `joblib.dump` to persist models and scalers for later use.

---

## 12) Troubleshooting

- **File not found:** Ensure the CSV is in `./data/` and your working directory is project‑root.
- **Column name mismatches:** Print `df.columns` and align code to your file (case‑sensitivity matters).
- **Mixed dtypes in numeric columns:** Use `pd.to_numeric(..., errors="coerce")` and handle `NaN`s.
- **Plots look empty:** Check that your `Date` is parsed as datetime and your series actually have data.
- **Model looks “too good” or “too bad”:** Verify you didn’t leak future data; prefer time‑aware splits.

---

## 13) Reproducibility

Set a seed where applicable (e.g., `np.random.seed(42)`, model `random_state=42`) and record your package versions:

```python
import sys, numpy, pandas, sklearn, matplotlib, seaborn
print(sys.version)
print(numpy.__version__, pandas.__version__, sklearn.__version__, matplotlib.__version__, seaborn.__version__)
```
...
This notebook is for educational purposes. If you use third‑party datasets, respect their licenses and cite sources as needed.
